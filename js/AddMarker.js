AFRAME.registerComponent("add-marker", {
    init: async function() {
        ...
        toys.map(toy => {
            ...
            var details = document.createElement("a-entity");
            details.setAttribute("id", `ingredients-${toy.id}`);
            details.setAttribute("position", { x: 0.3, y: 0, z: 0.1 });
            details.setAttribute("rotation", { x: 0, y: 0, z: 0 });
            details.setAttribute("text", {
                font: "monoid",
                color: "black",
                width: 2,
                align: "left",
                value: `${toy.details.join("\n\n")}`
            });
            mainPlane.appendChild(details);
